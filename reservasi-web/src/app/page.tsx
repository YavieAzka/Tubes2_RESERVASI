...
interface ApiResponse {
  success: boolean;
  data: Array<{ id: number; tag: string; text: string; attributes: Record<string, string>; depth: number }>;
...
function buildLayout(tree: Record<number, SerializedNode>): LayoutNode[] {
  ...
  // BFS layout
  const queue: Array<{ node: SerializedNode; depth: number }> = [{ node: root, depth: 0 }];
...
function getNodeColor(
  ...
  if (isCurrent && !isDone) return { fill: "#FBBF24", stroke: "#F59E0B", text: "#1a1a2e" };
  if (state === "match") return { fill: "#10B981", stroke: "#059669", text: "#fff" };
  if (state === "visit" && isDone) return { fill: "#6366f1", stroke: "#4F46E5", text: "#fff" };
  if (state === "backtrack") return { fill: "#7f1d1d", stroke: "#EF4444", text: "#FCA5A5" };
  if (state === "visit") return { fill: "#818CF8", stroke: "#6366F1", text: "#fff" };
...
export default function Home() {
  ...
  const [nodeStates, setNodeStates] = useState<Map<number, "visit" | "match" | "backtrack">>(new Map());
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!res.ok || !json.success) throw new Error(json.error || "Gagal scraping");
  ...
  // ── Animation engine ──────────────────────────────────────────────────────
  const playNext = useCallback((idx: number, steps: TraversalStep[], states: Map<number, "visit" | "match" | "backtrack">) => {
    if (idx >= steps.length) {
      setIsDone(true);
      setIsPlaying(false);
      return;
  ...
  //  Render 
  return (
    <main className="min-h-screen" style={{ background: "#0b0f1a", fontFamily: "'JetBrains Mono', 'Fira Code', monospace" }}>
      {/* Header */}
      <header style={{
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#e2e8f0", letterSpacing: "0.5px" }}>
            DOM Traversal Engine
          </h1>
          <p style={{ margin: 0, fontSize: 11, color: "#64748b" }}>IF2211 Strategi Algoritma · Tubes 2</p>
        </div>
        {result && (
          <span style={{
            marginLeft: "auto", fontSize: 11, padding: "4px 12px",
            borderRadius: 999, background: "#10b98122", color: "#10b981",
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "24px 24px" }}>
        ...
        <section style={{
          ...
          <h2 style={{ margin: "0 0 20px", fontSize: 13, fontWeight: 600, color: "#64748b", letterSpacing: "2px", textTransform: "uppercase" }}>
            Control Panel
          </h2>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {(["url", "html"] as const).map(m => (
              <button key={m} onClick={() => setInputMode(m)} style={{
                ...
                border: inputMode === m ? "1px solid #818cf8" : "1px solid #1e2d40",
              }}>
          ...
          {/* Row 2: main inputs */}
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
            {/* URL or HTML input */}
            <div>
              <label style={{ display: "block", fontSize: 11, color: "#64748b", marginBottom: 4, letterSpacing: "1px" }}>
            ...
            <div>
              ...
              <select value={algorithm} onChange={e => setAlgorithm(e.target.value as "DFS" | "BFS")} style={inputStyle}>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <select value={limitType} onChange={e => setLimitType(e.target.value as "all" | "top")} style={{ ...inputStyle, flex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
            <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 12, color: "#94a3b8" }}>
              <div
                ...
                <div style={{
                  position: "absolute", top: 2, left: animateOn ? 20 : 2,
            ...
            {animateOn && (
              <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "#94a3b8" }}>
                Kecepatan:
                <input
                  type="range" min={50} max={1000} step={50} value={animSpeed}
                  onChange={e => setAnimSpeed(Number(e.target.value))}
                  style={{ accentColor: "#6366f1", width: 100 }}
                />
                <span style={{ color: "#6366f1", minWidth: 45 }}>{animSpeed}ms</span>
              </label>
            )}
            ...
            <button
              ...
              style={{
                ...
                background: loading ? "#1e2d40" : "linear-gradient(135deg, #6366f1, #818cf8)",
                color: loading ? "#4b5563" : "#fff",
                border: "none", letterSpacing: "0.5px", transition: "all 0.2s",
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div
                onClick={() => { setLcaEnabled(v => !v); setLcaResult(null); }}
                style={{
                  width: 40, height: 22, borderRadius: 11,
                ...
                <div style={{
                  position: "absolute", top: 2, left: lcaEnabled ? 20 : 2,
                  width: 16, height: 16, borderRadius: 8,
                  background: "#fff", transition: "left 0.2s"
                }} />
              </div>
            ...
            {lcaEnabled && (
              <>
                <div>
                  <label style={{ display: "block", fontSize: 10, color: "#64748b", marginBottom: 3, letterSpacing: "1px" }}>NODE A (id)</label>
                  <input
                    type="number" min={0}
                    value={lcaNodeA}
                ...
                <div>
                  <label style={{ display: "block", fontSize: 10, color: "#64748b", marginBottom: 3, letterSpacing: "1px" }}>NODE B (id)</label>
                  <input
                    type="number" min={0}
                    value={lcaNodeB}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                  <label style={{ fontSize: 10, color: "#475569", letterSpacing: "1px" }}>
                    ⓘ lihat label <code style={{ color: "#818cf8" }}>id=N</code> di tree
                  </label>
                  <button
                    onClick={handleLCA}
                    disabled={lcaLoading || !result || lcaNodeA === "" || lcaNodeB === ""}
                    style={{
                      padding: "7px 20px", borderRadius: 7,
                      fontSize: 12, fontWeight: 700,
                      cursor: (lcaLoading || !result || lcaNodeA === "" || lcaNodeB === "") ? "not-allowed" : "pointer",
                      background: (lcaLoading || !result || lcaNodeA === "" || lcaNodeB === "")
                        ? "#1e2d40"
                        : "linear-gradient(135deg, #06B6D4, #0891B2)",
                      color: (lcaLoading || !result || lcaNodeA === "" || lcaNodeB === "") ? "#4b5563" : "#fff",
                      border: "none", boxShadow: "0 2px 10px #06B6D422",
                    }}>
                    {lcaLoading ? "⏳..." : "🔍 Cari LCA"}
                  </button>
                </div>
                ...
                {lcaResult && (
                  <div style={{
                    ...
                    background: "#080d18", border: "1px solid #06B6D433",
                    flexWrap: "wrap"
                  }}>
                    {lcaResult.lcaNodeId !== null ? (
                      <>
                        <span style={{ fontSize: 11, color: "#64748b" }}>LCA:</span>
                        <span style={{ fontSize: 13, fontWeight: 700, color: "#06B6D4", fontFamily: "monospace" }}>
                          &lt;{lcaResult.lcaTag}&gt; (id={lcaResult.lcaNodeId})
                        </span>
                        <span style={{ fontSize: 11, color: "#64748b" }}>path:</span>
                        <span style={{ display: "flex", alignItems: "center", gap: 4, flexWrap: "wrap" }}>
                          {(lcaResult.pathWithRole ?? lcaResult.pathNodeIds.map(id => ({ id, tag: "?", isLca: id === lcaResult.lcaNodeId }))).map((node, i, arr) => (
                            <span key={node.id} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                              <span style={{
                                padding: "2px 8px", borderRadius: 4, fontSize: 11,
                                fontFamily: "monospace", fontWeight: node.isLca ? 700 : 400,
                                background: node.isLca ? "#06B6D422" : "#F9731622",
                        ...
                        <button
                          ...
                          style={{
                            ...
                            color: "#475569", border: "1px solid #334155", cursor: "pointer"
                          }}>✕</button>
                      </>
                    ) : (
                      <span style={{ fontSize: 12, color: "#ef4444" }}>⚠ LCA tidak ditemukan</span>
                    )}
                  </div>
                )}
              </>
        ...
        {error && (
          <div style={{
            background: "#2d1515", border: "1px solid #ef444455",
        ...
        {result && (
          <>
            ...
            <div style={{
              ...
              {[
                { label: "Node Dikunjungi", value: result.stats.visitedCount, icon: "👁" },
                { label: "Elemen Cocok", value: result.stats.matchedCount, icon: "✅" },
                { label: "Waktu Cocok", value: `${result.stats.timeMs}ms`, icon: "⚡" },
                { label: "Kedalaman Maks", value: result.stats.maxDepth, icon: "🌳" },
              ].map(s => (
                <div key={s.label} style={{
                  background: "#0f172a", border: "1px solid #1e2d40",
                  borderRadius: 10, padding: "14px 18px"
                }}>
                  <div style={{ fontSize: 11, color: "#64748b", marginBottom: 4, letterSpacing: "1px" }}>{s.icon} {s.label}</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 20, marginBottom: 20 }}>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  <h3 style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#e2e8f0", letterSpacing: "1px" }}>
                    🌲 DOM Tree Visualization
                  </h3>
                  {/* Legend */}
                ...
                {animateOn && result.steps.length > 0 && (
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                    <button onClick={handlePlay} disabled={isPlaying} style={ctrlBtn(!isPlaying)}>▶ Play</button>
                    <button onClick={handlePause} disabled={!isPlaying} style={ctrlBtn(isPlaying)}>⏸ Pause</button>
                    <button onClick={handleReset} style={ctrlBtn(true)}>↺ Reset</button>
                    <div style={{ fontSize: 11, color: "#64748b", marginLeft: "auto" }}>
                      Step {Math.max(0, stepIndex + 1)} / {result.steps.length}
                      {isDone && <span style={{ color: "#10b981", marginLeft: 8 }}>✓ Selesai</span>}
                    </div>
                  </div>
                )}
                ...
                <div style={{ overflowX: "auto", overflowY: "auto", maxHeight: 480 }}>
                  <svg width={svgSize.w} height={svgSize.h} style={{ display: "block" }}>
                    ...
                    <defs>
                      <marker id="arrow-lca" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
                    ...
                    {layout.map(node => {
                      ...
                      const edgeActive = (parentState === "visit" || parentState === "match") &&
                      ...
                      return (
                        <line
                          ...
                          stroke={isLcaEdge ? "#F97316" : edgeActive ? "#6366f155" : "#1e2d40"}
                    ...
                    {/* Nodes */}
                    {layout.map(node => {
                      const colors = getNodeColor(node.id, currentStep, nodeStates, isDone, lcaResult);
                      const isActive = currentStep?.nodeId === node.id && !isDone;
                      const isLcaNode = lcaResult?.lcaNodeId === node.id;
                      const isLcaPath = lcaResult?.pathNodeIds.includes(node.id);
                      return (
                        <g key={node.id} style={{ transition: "all 0.3s" }}>
                          <circle
                            cx={node.x} cy={node.y} r={isActive ? 26 : isLcaNode ? 26 : 22}
                            fill={colors.fill}
                            stroke={colors.stroke}
                            strokeWidth={isActive ? 2.5 : isLcaNode ? 3 : isLcaPath ? 2 : 1.5}
                            style={{
                              transition: "all 0.2s",
                              filter: isActive ? "drop-shadow(0 0 8px #FBBF2488)"
                                : isLcaNode ? "drop-shadow(0 0 10px #06B6D488)"
                                : isLcaPath ? "drop-shadow(0 0 6px #F9731688)"
                                : "none"
                            }}
                          />
                          <text
                          ...
                          {/* match label */}
                          {nodeStates.get(node.id) === "match" && !isLcaNode && (
                            <text x={node.x} y={node.y - 30} textAnchor="middle" fontSize={9} fill="#10b981" fontFamily="monospace">match</text>
                          )}
                          {/* LCA label */}
                          {isLcaNode && (
                            <text x={node.x} y={node.y - 32} textAnchor="middle" fontSize={9} fontWeight={700} fill="#06B6D4" fontFamily="monospace">LCA ★</text>
                          )}
                          {/* LCA path label (non-LCA nodes on path) */}
                          {isLcaPath && !isLcaNode && (
                            <text x={node.x} y={node.y - 30} textAnchor="middle" fontSize={9} fill="#F97316" fontFamily="monospace">path</text>
                          )}
                        </g>
                      );
                    })}
              ...
              <div style={{
                ...
                <h3 style={{ margin: "0 0 12px", fontSize: 13, fontWeight: 600, color: "#e2e8f0", letterSpacing: "1px" }}>
                  📋 Traversal Log
                </h3>
                <div
                  ref={logRef}
                  style={{
                    flex: 1, overflowY: "auto", maxHeight: 430,
                    background: "#080d18", borderRadius: 8, padding: 12,
                    fontFamily: "monospace", fontSize: 11, color: "#64748b",
                    lineHeight: 1.8,
                  }}>
                  {(animateOn ? visibleLog : result.traversalLog).map((line, i) => {
                    const isMatch = line.includes("MATCH");
                    const isVisit = line.includes("Visit");
                    return (
                      <div key={i} style={{
                        color: isMatch ? "#10b981" : isVisit ? "#818cf8" : "#475569",
                        borderLeft: isMatch ? "2px solid #10b981" : "2px solid transparent",
                        paddingLeft: 8, marginBottom: 1
                      }}>
                        {line}
                      </div>
                    );
                  ...
                  {(!animateOn || isDone) && (
                    <div style={{ color: "#6366f1", marginTop: 8, borderTop: "1px solid #1e2d40", paddingTop: 8 }}>
                      — Traversal selesai —
                    </div>
                  )}
                </div>
            ...
            <div style={{
              ...
              <div style={{
                background: "#080d18", borderRadius: 8, padding: 16,
                maxHeight: 280, overflowY: "auto",
                fontFamily: "monospace", fontSize: 12,
                color: "#10b981", lineHeight: 1.7
              }}>
        ...
        {!result && !loading && !error && (
          <div style={{
            ...
            color: "#1e2d40", fontSize: 14
          }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>🌲</div>
            <p>Masukkan URL atau HTML, lalu klik <strong style={{ color: "#6366f1" }}>Mulai Pencarian</strong></p>
          </div>
        )}
      </div>
    </main>
...
const inputStyle: React.CSSProperties = {
  ...
  background: "#080d18", border: "1px solid #1e2d40",
